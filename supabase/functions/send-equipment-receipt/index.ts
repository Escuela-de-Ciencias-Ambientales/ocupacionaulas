...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const selected=units.map((u:any)=>`<tr><td>${esc(u.type)}</td><td>${esc(u.consecutive_code)}</td><td>${esc(u.asset_number||"—")}</td><td>${esc([u.brand,u.model].filter(Boolean).join(" · ")||"—")}</td><td>${esc(u.serial_number||"—")}</td><td>${u.returned_at?`Devuelto por: ${esc(u.returned_by)}${u.return_note?` · ${esc(u.return_note)}`:""}`:"Pendiente"}</td></tr>`);
