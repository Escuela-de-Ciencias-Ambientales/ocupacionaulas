...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: authorized, error: lookupError } = await adminClient
      .from('teacher_registry').select('id,full_name,unit,active,claimed_at').eq('email', email).maybeSingle();
    ...
    if (authorized.unit && authorized.unit !== unit) {
      return response({ ok: false, error: 'La unidad seleccionada no coincide con el registro administrativo. Contacta a la administración.' }, 403);
    }
